...
def replace_build_gradle():
    ...
    gitignore_base_path = os.path.abspath(os.path.join("../resources", ".gitignore"))
    ...
    if selected_folder and os.path.isdir(selected_folder):
        ...
        build_gradle_base_path = os.path.abspath(os.path.join("../resources", "build.gradle"))
        build_serenity_base_path = os.path.abspath(os.path.join("../resources", "serenity.conf"))
...
root = tk.Tk()
root.title("Generador de codigo")
...
file_menu.add_command(label="Comprobar links", command=open_file)
file_menu.add_command(label="Probar formularios", command=save_file)
file_menu.add_command(label="De POM a Screenplay", command=save_file)
...
label_path = tk.Label(frame_top, text="Ruta de la carpeta:", font=custom_font)
entry_path = tk.Entry(frame_top, width=40)
button_browse = tk.Button(frame_top, text="Buscar carpeta", command=open_folder_dialog, font=custom_font)
...
label_link = tk.Label(frame_top, text="Link:", font=custom_font)
...
entry_link.grid(row=1, column=1, padx=5, pady=5)
 ...
 # Crear el frame para contener el área de texto y el scrollbar
frame_bottom = tk.Frame(root)
frame_bottom.pack(fill="both", expand=True, padx=5, pady=5)

custom_font = ("Consolas", 13)

# Crear el widget de área de texto
text_area = tk.Text(frame_bottom, width=70, height=15, wrap="word", font=custom_font)  # La opción wrap="word" permite que las palabras se rompan al final de la línea
text_area.pack(side="left", fill="both", expand=True)

# Crear el objeto Scrollbar
scrollbar = tk.Scrollbar(frame_bottom, command=text_area.yview)

# Asociar el área de texto con el Scrollbar
text_area.config(yscrollcommand=scrollbar.set)

# Ubicar el scrollbar en el lado derecho y que ocupe todo el espacio vertical disponible
scrollbar.pack(side="right", fill="y")

text_area.tag_configure("green_text", foreground="green")
text_area.tag_configure("orange_text", foreground="#c07231")
...
text_area.tag_add("green_text", "1.0", "1.13")

# Encontrar la posición del texto "Característica:"
caracteristica_pos = text_area.search("Característica:", "1.0", stopindex="end")
...
radio_pom = tk.Radiobutton(frame_radio_buttons, text="Pom", variable=selected_option, value=1,font=custom_font)
radio_screenplay = tk.Radiobutton(frame_radio_buttons, text="Screenplay", variable=selected_option, value=2, font=custom_font)

# Ubicar los radio buttons uno al lado del otro en el frame
radio_pom.pack(side="left")
radio_screenplay.pack(side="left")

# Centrar el frame en la ventana
frame_radio_buttons.pack_configure(anchor="center")




# Crear el botón y ubicarlo debajo de los radio buttons
replace_button = tk.Button(root, text="Generar", command=replace_build_gradle, font=custom_font)
replace_button.pack(pady=10)  # Sin la opción "side" para que quede debaj

# Ejecuta la ventana
root.mainloop()

# Recordar separar la funcionalidad en modulos o lo muerde la vaca!